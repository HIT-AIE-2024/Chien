...
from config.configs import OptionalListStr, ListStr, TOKENIZATION_METHOD, ADJ, ADJ_SAT, ADV, NOUN, VERB
...
class TextPreprocessor:
    ...
    def stemming(self) -> ListStr:
        ...
        self.stemmed_tokens = [self.stemmer.stem(token) for token in self.tokens]
        return self.stemmed_tokens
    
    def lemmatization(self) -> ListStr:
        """
        ...
        self.lemmatized_tokens = [self.lemmatizer.lemmatize(token, pos=self.get_wordnet_pos(token)) for token in self.tokens]
