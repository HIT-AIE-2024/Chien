...
class TextPreprocessor:
    ...
    def tokenize(self, method: str = 'whitespace') -> L:
        """
        ...
        Returns:
            L: A list of tokens obtained after tokenization.
        """
    ...
    def stemming(self, tokens: L) -> L:
        """
        Performs stemming on the given tokens.

        Args:
            tokens (L): A list of tokens to perform stemming on.

        Raises:
            ValueError: If tokenization has not been performed before stemming.

        Returns:
            L: A list of stemmed tokens.
        """
    ...
    def lemmatization(self, tokens: L) -> L:
        """
        Performs lemmatization on the given tokens.

        Args:
            tokens (L): A list of tokens to perform lemmatization on.

        Raises:
            ValueError: If tokenization has not been performed before lemmatization.

        Returns:
            L: A list of lemmatized tokens.
        """
        if self.tokens is None:
            raise ValueError("Tokenization not performed. Please call the tokenize method first.")
